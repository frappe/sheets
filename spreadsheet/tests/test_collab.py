...
# Eagerly import the module under test so `mock.patch("spreadsheet.collab.frappe")`
# can resolve the attribute — the patcher's lazy import doesn't populate
# `spreadsheet.collab` on the parent package.
from spreadsheet import collab as _collab  # noqa: F401


def _patched_frappe():
	"""Patch ``spreadsheet.collab.frappe`` with a baseline-permissive mock."""
	patcher = mock.patch("spreadsheet.collab.frappe")
...
class CheckCollabAccess(unittest.TestCase):
	...
	def test_rejects_guest(self):
		from spreadsheet import collab
